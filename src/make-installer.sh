...
if [ ${PROJECT_URL} != "" -a ! -f ${WORKINGDIR}/README.txt ]; then
  echo "This is the installer tarball for ${APPLICATION}-${VERSION}" > ${TMPDIR}/README.txt
  echo "Project URL: ${PROJECT_URL}" >> ${TMPDIR}/README.txt
  echo "" >> ${TMPDIR}/README.txt
  echo "This tarball was generated by make-installer." >> ${TMPDIR}/README.txt
  README="README.txt"
fi
...
PREVWD=`pwd`
cd $TMPDIR
tar czf ${APPLICATION}-${VERSION}.tar.gz ${APPLICATION}-${VERSION} ${README}
mv *.tar.gz $PREVWD
cd $PREVWD
rm -rf ${TMPDIR}
